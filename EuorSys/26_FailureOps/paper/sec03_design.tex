...
The paired delta logical-failure rate is defined as $\Delta = (I - R) / N$, where $R$ is the rescued failure count, $I$ is the induced failure count, and $N$ is the total number of paired shots. A negative $\Delta$ indicates that the intervention rescues more failures than it induces; a positive $\Delta$ indicates the reverse.
...
\textbf{Decoder-pathway interventions} change the decoder algorithm or the prior distribution that feeds it. The primary decoder-pathway intervention in this paper switches from a correlated-matching decoder to a tesseract decoder, both using the same SI1000 prior (a uniform prior that assumes all error mechanisms are equally likely). This is a natural systems question: given the same detector records, does switching to a different decoder backend change which shots succeed and which fail? A complementary prior-switching intervention---keeping the tesseract decoder fixed and switching from SI1000 to frequency-calibrated prior---is evaluated in the expanded corpus (Section~5.6) and decoder-prior study (Section~5.7).
