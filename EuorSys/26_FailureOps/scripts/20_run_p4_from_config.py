#!/usr/bin/env python
"""Run P4 from a YAML config and write manifest-backed artifacts."""
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/p4_default.yaml")
                ...
                for baseline in run_rows:
                    for intervention in config["interventions"]:
                        ...
                        intervention_rows.append(
                            build_p4_intervention_row(baseline, intervened, intervention)
                        )
    ...
    print(f"wrote {len(baseline_rows)} P4 baseline rows to {outputs['baseline']}")
    print(f"wrote {len(intervention_rows)} P4 intervention rows to {outputs['interventions']}")
    print(f"wrote {len(validation_rows)} P4 validation rows to {outputs['validation']}")
    print(f"wrote {len(effect_rows)} P4 paired-effect rows to {outputs['effects']}")
    print(f"wrote P4 manifest to {outputs['manifest']}")
