#!/usr/bin/env python
"""Run the P5c rotated surface-code pilot through the P4 engine."""
...
from failureops.runtime_service import apply_p3_intervention
from failureops.surface_backend import generate_surface_code_runs


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--config", default="configs/p5c_surface_pilot.yaml")
    ...
    print(f"wrote {len(baseline_rows)} P5c baseline rows to {outputs['baseline']}")
    print(f"wrote {len(intervention_rows)} P5c intervention rows to {outputs['interventions']}")
    print(f"wrote {len(validation_rows)} P5c validation rows to {outputs['validation']}")
    print(f"wrote {len(effect_rows)} P5c paired-effect rows to {outputs['effects']}")
    print(f"wrote P5c manifest to {outputs['manifest']}")
