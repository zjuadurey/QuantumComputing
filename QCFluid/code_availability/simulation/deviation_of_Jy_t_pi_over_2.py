...
def build_circuit_deviation_of_Jy() -> simulator.Circuit:
    with open(root.joinpath('raw_circuit/diverging_flow_t=pi_over_2.qasm'), 'r') as f:
        qasm = f.read()
    qasm = qasm.split(';\n')
...
def deviation_of_Jy_simulation(save=False):
    ...
    if save:
        ...
        with pd.ExcelWriter(root.joinpath('temp/deviation_of_Jy_t_pi_over_2.xlsx')) as ew:
    ...
    exp_data_path = root.parent.parent.joinpath(
        "data_availability/Fig2/Exp_t=pi_over_2.xlsx")
